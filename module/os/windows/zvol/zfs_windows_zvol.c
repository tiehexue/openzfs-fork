...
ListLoadedDrivers(void)
{
	...
	if (windows_zvol_enabled == 1) {
		dprintf(
	"Aomei Backupper (ambakdrv.sys) detected as installed.\n"
	...
	"windows_zvol_enabled to 2 to force load OpenZFS with zvol support.\n");
		...
		dprintf("Force loading OpenZFS with zvol support, "
...
zvol_start(PDRIVER_OBJECT DriverObject, PUNICODE_STRING pRegistryPath)
{
	...
	hwInitData.HwInitialize =
	    (PHW_INITIALIZE) wzvol_HwInitialize;
	hwInitData.HwStartIo =
	    (PHW_STARTIO) wzvol_HwStartIo;
	hwInitData.HwFindAdapter =
	    (PVIRTUAL_HW_FIND_ADAPTER) wzvol_HwFindAdapter;
	hwInitData.HwResetBus =
	    (PHW_RESET_BUS) wzvol_HwResetBus;
	hwInitData.HwAdapterControl =
	    (PHW_ADAPTER_CONTROL) wzvol_HwAdapterControl;
	hwInitData.HwFreeAdapterResources =
	    (PHW_FREE_ADAPTER_RESOURCES) wzvol_HwFreeAdapterResources;
	hwInitData.HwInitializeTracing = wzvol_TracingInit;
	hwInitData.HwCleanupTracing = wzvol_TracingCleanup;
	hwInitData.HwProcessServiceRequest =
	    (PHW_PROCESS_SERVICE_REQUEST) wzvol_ProcServReq;
	hwInitData.HwCompleteServiceIrp =
	    (PHW_COMPLETE_SERVICE_IRP) wzvol_CompServReq;
