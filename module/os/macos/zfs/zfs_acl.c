...
zfs_acl_xform(znode_t *zp, zfs_acl_t *aclp, cred_t *cr)
{
	...
	aclp->z_ops = &zfs_acl_fuid_ops;
	VERIFY(zfs_copy_ace_2_fuid(zp->z_zfsvfs, vnode_vtype(ZTOV(zp)), aclp,
...
zfs_acl_chmod_setattr(znode_t *zp, zfs_acl_t **aclp, uint64_t mode)
{
	...
	if (error == 0) {
		...
		zfs_acl_chmod(vnode_vtype(ZTOV(zp)), mode, B_TRUE,
    ...
    zidmap_t *mnt_ns)
{
	...
	is_attr = ((zp->z_pflags & ZFS_XATTR) &&
	    (vnode_vtype(ZTOV(zp)) == VDIR));

#ifdef __APPLE__
	/*
	 * In APPLE, we don't care about permissions of individual ADS.
...
zfs_zaccess_delete(znode_t *dzp, znode_t *zp, cred_t *cr, zidmap_t *mnt_ns)
{
	...
	if ((dzp_error = zfs_zaccess_common(dzp, ACE_DELETE_CHILD,
	    &dzp_working_mode, &dzpcheck_privs, B_FALSE, cr, mnt_ns)) == 0)
		return (0);
	...
	if ((zp_error = zfs_zaccess_common(zp, ACE_DELETE, &zp_working_mode,
	    &zpcheck_privs, B_FALSE, cr, mnt_ns)) == 0)
		return (0);
    ...
    znode_t *tzp, cred_t *cr, zidmap_t *mnt_ns)
{
	...
	add_perm = (vnode_vtype(ZTOV(szp)) == VDIR) ?
	 ...
	 * to another.
	 */
	if (vnode_vtype(ZTOV(szp)) == VDIR && ZTOV(sdzp) != ZTOV(tdzp)) {
